...
3. `StorageBallotProposalV1`
   - epoch, generation, 64-bit cursor/slot, `(counter, coordinatorId16)` ballot;
   - previous commit hash, logical object ID, request digest and exact tombstone target.
4. `StorageEpochOperationBindingV1`
   ...
   - binding to network, epoch, membership/placement, generation, cursor, previous commit and
     operation digest.
5. `StorageCommitEnvelopeV1`
   - canonical record/tombstone body, previous/current commit hashes and embedded P03B `MQR1`;
   - verifier checks two exact HRW owner IDs and caller expectations.
6. `StorageReplicaHighWaterV1`
   - replica ID, epoch, generation, committed cursor, commit hash, issued bucket and replica
     signature.
7. `LegacyMirrorEnvelopeV1` and `LegacyMirrorReceiptV1`
