...
public sealed class PeerEndpointPolicyTests
{
    ...
    [Fact]
    public void ExactPrivateTuple_AuthorizesCanonicalMailboxRouteOnSamePeerAuthority()
    {
        var recipient = Id(2);
        var policy = CreateUatPolicy(recipient, "10.20.30.40", 8081);
        var mailboxEndpoint = new Uri("http://10.20.30.40:8081/api/peer/mailbox/v2/store");

        Assert.True(policy.TryValidatePeerRouteEndpoint(
            recipient,
            mailboxEndpoint,
            "/api/peer/mailbox/v2/store",
            out _));
        Assert.True(policy.IsResolvedAddressAllowed(
            recipient,
            mailboxEndpoint,
            "/api/peer/mailbox/v2/store",
            IPAddress.Parse("10.20.30.40")));
        Assert.False(policy.TryValidatePeerRouteEndpoint(
            Id(3),
            mailboxEndpoint,
            "/api/peer/mailbox/v2/store",
            out _));
        Assert.False(policy.TryValidatePeerRouteEndpoint(
            recipient,
            new Uri("http://10.20.30.40:8081/api/peer/mailbox/v2/tombstone"),
            "/api/peer/mailbox/v2/store",
            out _));
    }
